...
class SARSAQNetwork:
    def __init__(self, observation_space_size, action_space_size):
        ...
        observable_input = tf.keras.layers.Input(shape=(observation_space_size))
        epss_input = tf.keras.layers.Input(shape=(observation_space_size, observation_space_size))
        ntpg_input = tf.keras.layers.Input(shape=(observation_space_size, observation_space_size))
        ...
        # model output
        output = tf.keras.layers.Dense(action_space_size, activation='softmax')(x)

        # Create model
        self.model = keras.Model(inputs=[observable_input, epss_input, ntpg_input], outputs=output)
    ...
    def predict(self, observation, epss_input, ntpg_input):
        # Forward pass through the network to get Q-values
        # print("Raw prediction: ", self.model.predict(observation))
        return self.model.predict([observation, epss_input, ntpg_input])
        

# a value ambiguous use a.any or a.all fix: if load a model, put it in square bracket because my model take a list of input.
# like this: self.model = keras.Model(inputs=[observable_input, epss_input, ntpg_input], outputs=output)
...
class SarsaLearning:
    def __init__(self, env, epsilon, numberEpisodes, max_steps, alpha, gamma, total_permutations, fnr, fpr):
        ...
        self.actionDimension = factorial(self.env.K) / (factorial(self.env.K - env.M) * factorial(self.env.M))
    ...
    def selectAction(self, state):
        ...
        q_values = self.mainNetwork.predict(state, self.epss_input_reshaped, self.ntpg_input_reshaped)[0]
    ...
    def selectActionEval(self, state, episode, trained_model):
        state = state.reshape(1, -1)
        q_values = trained_model.predict([state, self.epss_input_reshaped, self.ntpg_input_reshaped])[0]
    ...
    def updateQvalues(self, state1, action1, reward, state2, action2, done):
        ...
        current_q_values = self.mainNetwork.predict(state, self.epss_input_reshaped, self.ntpg_input_reshaped)
        next_q_values = self.mainNetwork.predict(state2, self.epss_input_reshaped, self.ntpg_input_reshaped)
        ...
        self.mainNetwork.model.fit([state, self.epss_input_reshaped, self.ntpg_input_reshaped], target_vector, epochs=1, verbose=0)
    ...
    def saveModel(self):
        if os.name == 'nt':
            ...
            self.mainNetwork.model.save(f"./TrainedModel/weighted_random_attacker/SARSA_3_INPUT/RL_Honeypot_SARSA_win_ver{self.getStepCount()}_{token}.keras")
            self.updateModelPath(f"./TrainedModel/weighted_random_attacker/SARSA_3_INPUT/RL_Honeypot_SARSA_win_ver{self.getStepCount()}_{token}.keras")
        else:
            os.makedirs("./TrainedModel/weighted_random_attacker/SARSA_3_INPUT", exist_ok=True)
            token = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))
            self.mainNetwork.model.save(f"./TrainedModel/weighted_random_attacker/SARSA_3_INPUT/RL_Honeypot_SARSA_linux_ver{self.getStepCount()}_{token}.keras")
            self.updateModelPath(f"./TrainedModel/weighted_random_attacker/SARSA_3_INPUT/RL_Honeypot_SARSA_linux_ver{self.getStepCount()}_{token}.keras")
